...
def finn_plan_med_id(planer, søkt_id): #Oppg2
    for plan in planer:
        if plan.id == søkt_id:
            return plan
    return None

def meny(emner, planer): #Oppg2
...
def tom_studieplan(plan_id, tittel, antall_semestre=6): #Oppg5
    emner_per_semester = {}
...
def tom_studieplan_obj(plan_id, tittel, antall_semestre=6): #Oppg5
    plan = Studieplan(plan_id, tittel)
    for sem in range(1, antall_semestre + 1):
        plan.emner_per_semester[sem] = []
    return plan

def lag_tom_studieplan(): #Oppg5
...
def finn_planer_for_emne(): #Oppg8
    studieplan_med_emne = []
    onsket_emne = input("Oppgi emne du ønsker å søke opp: ").upper()
    for plan in studieplaner:
        if onsket_emne in plan:
            studieplan_med_emne.append(plan)
            print(f"Plan: {plan.navn}")
    
    if not studieplan_med_emne:
        print("Emnet er ikke lagt i noen studieplan ennå")
...
menyvalg_funksjoner = {
    ...
    2: lambda: kjør_valg(lambda: meny(emner, studieplaner)),
